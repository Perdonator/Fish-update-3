...
function casino() {
	let value = parseInt(document.getElementById("casinovalue").value)
	let won = document.getElementById("youwon")
	let ranum = Math.floor(Math.random() * 10)
	if (score >= value && value >= 100) {
		score = score - value
	    updateScore()
		if (ranum <= 6) {
			...
			score = score + value
			updateScore()
			won.innerHTML = "You won nothing, omg!";
		} else {
			if (ranum <= 5) {
				Coins = Coins + Math.round((value * 0.02));
				localStorage.setItem("Coins", Coins);
				won.innerHTML = "You won X2"
				localStorage.setItem("Coins", Coins)
				update()
			} else if (ranum < 9) {
				Coins = Coins + Math.round((value * 0.03));
				localStorage.setItem("Coins", Coins);
				won.innerHTML = "You won X3"
				document.getElementById("coins").innerHTML = Coins;
				localStorage.setItem("Coins", Coins)
				update()
			} else  {
				Coins = Coins + Math.round(value * 0.05);
				localStorage.setItem("Coins", Coins);
				won.innerHTML = "JACKPOT!"
				document.getElementById("coins").innerHTML = Coins;
				localStorage.setItem("Coins", Coins)
				update()
		}			
		...
		alert("Not enough score or too low bid!")
	}
...
function reset() {
	if (confirm('Are you sure you want to reset all your progress?')) {
		...
		updateScore();
		update();
		localStorage.setItem("FishWallaper1", false);
		localStorage.setItem("clickerScore", score);
		localStorage.setItem("ClickGrade", clickGrader);
		localStorage.setItem("Pay", pay);
